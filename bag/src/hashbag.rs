...
pub struct Bag<K, V>(std::collections::HashMap<K, Vec<V>>);
...
impl<K, V> From<Bag<K, V>> for std::collections::HashMap<K, Vec<V>> {
    fn from(value: Bag<K, V>) -> std::collections::HashMap<K, Vec<V>> {
        value.into_inner()
    }
}
